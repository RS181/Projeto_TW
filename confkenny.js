//Contole de vez (0  == vez do Humano )||(1 == vez da IA)
var turn_control = 0;
var choosen_dificulty = 0;
// CPU == 1 ; ON
var cpu = 0;
var teste = 0;
var alertshown = 1;
...
class Tabuleiro {
        ...
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                ...
                child.addEventListener("click", async (event) => {
                    //  console.log("============== "+this.nr_pecas_brancas , this.nr_pecas_pretas);
                    if (this.GamePhase == "DropPhase"){ 
                        //Para não clicar numa casa já preenchida
                        if(event.target.className == "peca_tabuleiro_preta" || event.target.className == "peca_tabuleiro_branca" || event.target.childElementCount != 0){
                            alert("Tile preenchido");
                            return;
                        }
                        if(turn_control == 0 || cpu == 0)
                            await this.play(event.target.id);


                        /*//Ciclo em baixo quebra quando movemos a peça para outra casa
                        while (this.teste == false && turn_control == 1) {
                        ...
                        */
                        ...
                        //turn_control = 0;
                        //alert(turn_control);
                        if(cpu == 1 && turn_control == 1 && this.nr_pecas_pretas == this.nr_pecas_brancas-1){
                            // Só para o CPU não jogar muito rápido
                            //await sleep(800);
                            this.cpuMove();
                        }
                        if(this.nr_pecas_brancas == 0 && this.nr_pecas_pretas == 0 && this.alertshown == 1){
                            this.GamePhase == "MovePhase"
                          //  alert("Move phase");
                            //alertshown = 0;
                        }
                    } else if (this.GamePhase == "MovePhase") {
                        if(turn_control == 1 || turn_control == 0 || cpu == 0){
                            await this.move_phase(event);
                            turn_control++;
                        }
                    let dummy = 0;
                        if(cpu == 1 && turn_control == 2){
                           // await sleep(5000);
                            await this.cpu_MovePhase();
                            turn_control = 0;
                        }
                    }
                });
    ...
    cpuMove(){
        if(this.GamePhase = "DropPhase"){
            var filtered = [];
            parent = document.querySelectorAll(".item_tabuleiro");
            console.log("CPU move");
    
            parent.forEach( (element) => {
                if (this.Posicao_valida(element.id, "branca") == true && element.childElementCount == 0) {
                    filtered.push(element);
                }
            });
    
            var local = randomize(filtered);
            console.log(local);
            this.por_peca(local);
    
            //this.Eliminar_peca_resolvido = false;
           // this.Eliminar_peca(local.id, "branca");
    
            return local;
        }else if(this.GamePhase = "MovePhase"){
            alert("NOVO : entrou aqui");
        }
    }

    async cpu_MovePhase(){
        let white_pos = document.querySelectorAll("div.item_tabuleiro > div.peca_tabuleiro_branca");
        let black_pos = document.querySelectorAll("div.item_tabuleiro > div.peca_tabuleiro_preta");
        
        let temp_stack=[];
        let i= 0;
        let colunas = parseInt(this.cols);
        for(let element of white_pos){
            let parent_id = element.parentNode.id;
            console.log(parent_id);
            let left = parseInt(parent_id)-1;
            let right = parseInt(parent_id)+1;
            let up = parseInt(parent_id) - colunas;
            let down = parseInt(parent_id) + colunas;

            console.log(left%6 + " " + parseInt(parent_id)%colunas);
            console.log(right%6 + " " + parseInt(parent_id)%colunas);
            console.log(up);
            console.log(down);
            
            //console.log(left%colunas + " " + parseInt(parent_id)%colunas);
            //console.log(document.getElementById(left) == null);
            //console.log(document.getElementById(left).childElementCount == 0);

            if((parseInt(parent_id)%colunas == 1 && left%colunas == 0) || document.getElementById(left) == null){
                left = 0;
            }else if(document.getElementById(left).childElementCount == 1){
                left = 0;
            }

            if((parseInt(parent_id)%colunas == 0 && right%colunas == 1) || document.getElementById(right) == null ){
                right = 0;
            } else if( document.getElementById(right).childElementCount == 1){
                right = 0;
            }

            if(up < 0 || document.getElementById(up) == null){
                up = 0;
            }else if(document.getElementById(up).childElementCount == 1){
                up = 0;
            }

            if(down > colunas*parseInt(this.rows) || document.getElementById(down) == null ){
                down = 0;
            }else if(document.getElementById(down).childElementCount == 1 ){
                down = 0;
            }
            temp_stack[i] = [left,right,up,down,parent_id];
            console.log(temp_stack[i]);
            i++;
        }
        let filtered = shuffle(temp_stack)[0];
        let source = document.getElementById(filtered[4]);
        let destiny = document.getElementById(filtered.slice(0, 4)[Math.floor(Math.random()*4)]);
        destiny.appendChild(source.firstChild);
        console.log("SOURCE " + source.firstChild);
    ...
    */
    ...
    async move_peca(cor_peca) {
        return new Promise(async (resolve,reject) => {
            ...
            */
            ...
            if(this.Eliminar_peca_resolvido == true)
                turn_control =2;
    ...
    async Eliminar_peca(Posicao, cor_peca) {
        ...
        if ((aux[0] == 3 && this.used_linha[aux[2]] == false) ||
            ...
            if(turn_control == 1 /*&& (turn_control == 0 || cpu == 0)*/){
                //let divs = document.getElementsByClassName('.item_tabuleiro');
                for (let peca of divs) {
                   // console.log(peca + " " + peca.className == peca_a_remover);
                    if (peca.className == peca_a_remover) {
                        peca.onclick = function () {
                            console.log("linha 669 : " + peca);
                            peca.parentNode.removeChild(peca);
                        }
                        peca.style["border-color"] = "red";
                    }
                }
            }else if(cpu = 1 && turn_control == 0){
                let peca = 0;
                let para_remover =[];
                for (peca of divs) {
                    if (peca.className == peca_a_remover) {
                       // alert("entra aqui");
                        console.log(peca.className + " --------------------------- ");                    
                        peca.style["border-color"] = "red";
                        console.log("linha 669 : " + peca);
                        para_remover.push(peca);
                    }
                }
                console.log(para_remover[0].parentNode.id);
                let temp = shuffle(para_remover)[0];
                temp.parentNode.removeChild(para_remover[0]);
            }
...
function randomize(parent){
   ...
   // parent = document.querySelectorAll(".item_tabuleiro");
    //console.log(parent);
    aux = [];
    for(let tile of parent){
        if(tile.childElementCount == 0){
            aux.push(tile);
        }
    }
    retorno = shuffle(aux)[0];
    //retorno = aux[0];
    console.log(retorno.id);
    /*if(color == ".peca_tabuleiro_preta"){*/
    //return retorno.id;
    return aux[0].id;
...
function shuffle(list) {
    ...
    while (currentIndex > 0) {
      ...
      [current[currentIndex], current[randomIndex]] = [
        current[randomIndex], current[currentIndex]];
    }
    return current;
}
...
//Função que decide qual div criar 
...
function create_div_Computer(parentid) {
    ...
    //!Por acabar 
    for(let checked in fieldset){
        console.log(this.choosen_dificulty);
        if(checked.checked){
            this.choosen_dificulty = checked;
            console.log(choosen_dificulty);
            break;
        }
    }
}
